...
class FlatMap {
private:
    std::vector<std::pair<Key, Value>> storage_;
    auto findIt(const Key& key) const {
        const auto& it = std::lower_bound(storage_.begin(), storage_.end(), key, [](const std::pair<Key, Value> &a, const Key &key) {
...
public:
    const Value* find(const Key& key) const {
        ...
        if (it == storage_.end() || Comp()(key, it->first)) {
            return nullptr;
        }
        return &it->second;
    }
    ...
    bool insert(Key key, Value value) { 
        ...
        storage_.emplace(it, std::move(key), std::move(value));
        return true;
    }
    ...
    public:
        ...
        reference operator*() const {
            auto &tmp = fmap_->storage_[i];
            return {tmp.first, tmp.second};
        }
        reference operator[](difference_type n) const {
            auto &tmp = fmap_->storage_[i+n];
            return {tmp.first, tmp.second};
        }
        pointer operator->() const {
            auto &tmp = fmap_->storage_[i];
            return ProxyBox {Proxy {tmp.first, tmp.second}};
        }
    };
    ...
    iterator end() {
        return iterator(this, storage_.size());
    }
